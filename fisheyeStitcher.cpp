...
main( int argc, char** argv )
{
    ...
    string video_path = "";
    float  fovd = 195.0f;
    bool   disable_light_compen = false;
    bool   disable_refine_align = false;
    int    fr_from = 1;
    int    fr_to = 1;
	...
	VideoCapture VCap("360_0033[1].mp4");
    ...
    if( !VCap.isOpened() )
    {
        CV_Error_(Error::StsBadArg, ("Error opening Cap video: %s",
                    video_path.c_str()));
		cout << "here's something?" << endl;
    } 
   ...
   /* VOut.open( video_out_name, VideoWriter::fourcc('X','2','6','4'),
    ...
    /*if( !VOut.isOpened() )
    {
		cout << "or here's something else?" << endl;
        CV_Error_(Error::StsBadArg, 
                  ("Error opening Out video: %s", video_out_name.c_str()));
    } */
	cout << "actually not okay" << endl;
    cv::Mat pano;
    ...
    {
        VCap >> imgorig;
		//imgorig = imread("360_0026.jpg");
		Mat img(1920, 3840, pano.type());
		resize(imgorig, img, img.size(), 0, 0, INTER_LINEAR);
        if( img.empty())
        {
		...
		imwrite("in_img_L.jpg", in_img_L);
		imwrite("in_img_R.jpg", in_img_R);

		cout << "img is ready" << endl;

        double startOneFrTime = double(getTickCount()); // frame stitching starts
		cout << "ready to stitch" << endl;
        //--------------------------------------------------------------
        // Stitch
        //--------------------------------------------------------------
		...
		cout << "ready to make pano" << endl;
		imwrite("isThisResultPano.jpg", pano);
		cout << "now our img is done" << endl;
