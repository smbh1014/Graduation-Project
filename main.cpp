...
main(int argc, char** argv)
{
	...
	fish_common_init(map_x, map_y, cir_mask, scale_map, mls_map_x, mls_map_y,
		binary_mask, blend_post, Hs, Ws, Hd, Wd, 97);
	// 
	in_img.release();
	...
	Mat img = imread("sam_original.jpg", 1);
	Mat convert_img;
	imshow("origianl", img);
	cv::Mat pano;
	...
	//--------------------------------------------------------------------------
		...
		imwrite("justResult.jpg", pano);
		cout << "problem occurs" << endl;

		Mat dst;
		//cv::resize(pano, dst, Size(1280, 720), 0, 0, CV_INTER_AREA);
		cv::resize(pano, dst, Size(1280, 720), 0, 0, 3);
		imshow("convert", dst);
		//imshow("Convert", pano);
